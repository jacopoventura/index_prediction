...
from src.random_forest.helpers_random_forest import *
...
# Training: process for minimizing the Cost function C using gradient descent: w_(k+1) = w_(k) - Lr * dC/dw
# Process to train the model using the chain rule.
# STEP 1: predict the output using the most recent updated model. The model's output will be compared to the expected value y.
# Then, the error is calculated using, for example, MSE.
# Through this error, the model “knows” how much to adjust its parameters in order to get closer to the expected output.
# STEP 2: apply the chain rule to compute the gradient dC/dw
# dC/dw = dC/da_i * da_i/dw where a_i = W_i * x + b, thus da_i / dw = w_i
# dC/da_i = dC/df_i * df_i/da_i where f_i activation of the ith layer. df_i/da_i is known, while dC/df_i can be further derived
# with the chain rule until can be calculated using C (loss) and the expected output.
# NOTE: ready, well-known model have the chain rule already implemented and the gradient is calculated as function of C and expected y.
# ##################################################################################################################
...
#
# Summary:
# RNNs are simpler and faster to train than LSTMs, as they have fewer parameters and computations.
# LSTMs can learn more complex and long-range patterns because of the long.memory pattern.
# RNNs have a limited memory capacity, while LSTMs can selectively remember or forget the relevant information.
# Finally, RNNs suffer from vanishing/exploding gradient during training.
# ##################################################################################################################
...
if __name__ == '__main__':
    ...
    # Random Forest are trained with Scikit-Learn, which can parallelize the training.
    for key, predictors in dict_predictors.items():
